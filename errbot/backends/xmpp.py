...
try:
    ...
    log.fatal(
        "If you intend to use the XMPP backend pleas install the support for XMPP with: " "pip install errbot[XMPP]"
    )
    sys.exit(-1)
...
class XMPPRoom(XMPPIdentifier, Room):
    ...
    def create(self):
        ...
        """
        logging.warning(
            "XMPP back-end does not support explicit creation, joining room " "instead to ensure it exists."
        )
    ...
    def configure(self):
        ...
        while affiliation is None:
            ...
            affiliation = self.xep0045.getJidProperty(
                room=room, nick=self.xep0045.ourNicks[room], jidProperty="affiliation"
            )
        ...
        if affiliation == "owner":
            ...
            log.debug(
                "Not configuring room {} because we don't have owner affiliation (affiliation={})".format(
                    room, affiliation
                )
            )
